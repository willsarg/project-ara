...
def _ollama_memory_snapshot(*, available_gib=8):
    ...
    return cli.ollama_evidence.MemorySnapshot(
        ...
        system_available_bytes=available_gib * gib,
        accelerator_kind="apple",
        accelerator_count=1,
        accelerator_total_bytes=None,
        accelerator_available_bytes=None,
        unified=True,
    )


def _wire_ollama_probe(monkeypatch, snapshots=None):
    monkeypatch.setattr(cli.ollama, "probe_generate", lambda _name, _ctx: True)
...
def test_ollama_measure_ceiling_finds_the_wall(monkeypatch):
    ...
    _wire_ollama_probe(
        monkeypatch,
        [_ollama_memory_snapshot()] * 5 + [_ollama_memory_snapshot(available_gib=1)],
    )
    monkeypatch.setattr(
        cli.ollama, "probe_generate",
        lambda _name, ctx: requested.append(ctx) or True,
    )
    ...
    assert requested == [2048, 4096, 8192]
...
def test_ollama_measure_ceiling_accepts_safe_partial_offload(monkeypatch):
    _wire_ollama_probe(monkeypatch)
...
def test_ollama_measure_ceiling_records_bounded_generation_failure(monkeypatch):
    monkeypatch.setattr(cli.ollama, "create", lambda p, m, ctx: True)
    monkeypatch.setattr(cli.ollama, "probe_generate", lambda p, ctx: False)
    monkeypatch.setattr(
        cli.ollama_evidence, "capture_memory_snapshot", _ollama_memory_snapshot)
    monkeypatch.setattr(
...
def test_ollama_measure_ceiling_stops_at_preload_capacity_gate(monkeypatch):
    monkeypatch.setattr(
        cli.ollama_evidence, "capture_memory_snapshot",
        lambda: _ollama_memory_snapshot(available_gib=3))
    monkeypatch.setattr(
...
def test_ollama_measure_ceiling_accepts_safe_discrete_partial_offload(monkeypatch):
    gib = 1024 ** 3
    before = cli.ollama_evidence.MemorySnapshot(
        32 * gib, 24 * gib, "nvidia", 1, 8 * gib, 7 * gib, False)
    after = cli.ollama_evidence.MemorySnapshot(
        32 * gib, 20 * gib, "nvidia", 1, 8 * gib, 2 * gib, False)
    _wire_ollama_probe(monkeypatch, [before, after])
    monkeypatch.setattr(cli.ollama, "create", lambda p, m, ctx: True)
    monkeypatch.setattr(cli.ollama, "ps", lambda *_a: [{
        "name": "pr", "context_length": 2048,
    ...
    assert points[0]["applicable_walls"] == ["system", "accelerator"]


def test_ollama_measure_ceiling_refuses_runtime_authority_drift(monkeypatch):
    endpoint = cli.ollama.OllamaEndpoint("http://127.0.0.1:11434", "loopback")
    authority = cli.ollama.OllamaRuntimeAuthority(
        endpoint=endpoint, server_version="0.30.10", server_instance_id="one",
        configured_num_parallel=1, configured_num_parallel_authority="exact_version_default")
    drifted = dataclasses.replace(authority, server_instance_id="two")
    _wire_ollama_probe(monkeypatch)
    monkeypatch.setattr(cli.ollama, "create", lambda p, m, ctx: True)
...
def test_ollama_measure_ceiling_refuses_runtime_drift_after_generation(monkeypatch):
    ...
    authority = cli.ollama.OllamaRuntimeAuthority(
        endpoint=endpoint, server_version="0.30.10", server_instance_id="one",
        configured_num_parallel=1, configured_num_parallel_authority="exact_version_default")
    observations = iter([authority, dataclasses.replace(authority, server_instance_id="two")])
    _wire_ollama_probe(monkeypatch)
    monkeypatch.setattr(cli.ollama, "create", lambda p, m, ctx: True)
...
def test_ollama_measure_ceiling_preserves_lower_fit_when_previous_runner_remains(monkeypatch):
    ...
    assert best == 2048
    assert [point["fit"] for point in points] == [True, False]
    assert points[1]["refusal_reasons"] == ["effective_residency_unverified"]
...
def test_ollama_measure_ceiling_binds_probe_and_base_manifest_before_load(monkeypatch):
    ...
    loads = []
    monkeypatch.setattr(
        cli.ollama, "probe_generate", lambda name, ctx: loads.append((name, ctx)) or True)
    monkeypatch.setattr(cli.ollama, "ps", lambda *_a: [{
...
def _local_ollama_model(name, **fields):
    fields.setdefault("size_bytes", 500_000_000)
...
def test_characterize_ollama_measures_and_records(store, monkeypatch, capsys):
    ...
    snapshot = _ollama_memory_snapshot()
    point = cli.ollama_evidence.characterization_point(
        snapshot,
        snapshot,
        cli.ollama.OllamaProcess(
            name="probe", size_bytes=1000, size_vram_bytes=1000,
    ...
    with cli.db.connected() as con:
        ...
        assert row["config"]["effective_per_request_context"] == 4096
        assert row["config"]["placement"] == "unified"
...
def test_characterize_ollama_refuses_if_runtime_authority_changes_before_store(
        store, monkeypatch, make_console):
    _wire_char_ollama(monkeypatch)
    endpoint = cli.ollama.OllamaEndpoint("http://127.0.0.1:11434", "loopback")
    initial = cli.ollama.OllamaRuntimeAuthority(
        endpoint=endpoint,
        server_version="0.30.10",
    ...
    observations = iter([initial, dataclasses.replace(initial, server_instance_id="two")])
    monkeypatch.setattr(cli.ollama, "runtime_authority", lambda _endpoint: next(observations))
    monkeypatch.setattr(
        cli, "_ollama_measure_ceiling", _fake_ollama_measure((4096, [])))

    c, buf = make_console()
    assert cli.render_characterize(c, "qwen3:0.6b", engine="ollama") == 1
    assert "runtime authority changed" in buf.getvalue()
    with cli.db.connected() as con:
        assert cli.db.get_characterization(con, "mk", "ollama", "qwen3:0.6b") is None
...
def test_characterize_ollama_refuses_unknown_model_size_before_probe(
        store, monkeypatch, make_console):
    _wire_char_ollama(monkeypatch)
    monkeypatch.setattr(
        cli.ollama, "inventory",
        lambda t=2.0: [_local_ollama_model(
            "qwen3:0.6b", digest="a" * 64, size_bytes=None)],
    )
    monkeypatch.setattr(
        cli, "_ollama_measure_ceiling", lambda *_a, **_k: pytest.fail("loaded unknown size"))

    c, buf = make_console()
    assert cli.render_characterize(c, "qwen3:0.6b", engine="ollama") == 1
    assert "on-disk size" in buf.getvalue() and "unbounded" in buf.getvalue()


def test_characterize_ollama_refuses_oversized_model_before_probe(
        store, monkeypatch, make_console):
    _wire_char_ollama(monkeypatch)
    gib = 1024 ** 3
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        cli.ollama, "inventory",
        lambda t=2.0: [_local_ollama_model(
            "qwen3:0.6b", digest="a" * 64, size_bytes=2 * gib)],
    )
    monkeypatch.setattr(
        cli, "_ollama_measure_ceiling", lambda *_a, **_k: pytest.fail("loaded oversized model"))
...
def test_characterize_ollama_handles_create_failure_without_cleanup(
    ...
    assert "no physically safe ceiling" in buf.getvalue()
...
def _wire_serve(monkeypatch, *, version="0.30.10", names=("qwen3:0.6b",), create_ok=True,
                ps_rows=None, characterization=Ellipsis, isatty=False):
    ...
    monkeypatch.setattr(
        cli.ollama, "pull", lambda *_a, **_k: pytest.fail("serve pulled model"))
    ...
    def inventory(timeout=2.0):
        ...
        return [_local_ollama_model(
            n,
            digest=("a" * 64 if n == "qwen3:0.6b" else "b" * 64),
        ) for n in names]
    ...
    monkeypatch.setattr(cli.profile, "machine_key", lambda: "mkey")
    if characterization is Ellipsis:
        characterization = {"safe_context": 8192}
    if characterization is not None and "artifact_id" not in characterization:
...
def test_serve_treats_implicit_latest_as_installed(make_console, monkeypatch):
    _wire_serve(
        monkeypatch, names=("qwen3:latest",), ps_rows=[],
        characterization={"safe_context": 8192,
                          "artifact_id": "ollama-manifest-sha256:" + "b" * 64})
    monkeypatch.setattr(
        cli.ollama, "pull",
...
def test_serve_refuses_missing_model_without_implicit_pull(make_console, monkeypatch):
    _wire_serve(monkeypatch, names=("other:1",))
    monkeypatch.setattr(cli.ollama, "pull", lambda *_a, **_k: pytest.fail("serve pulled model"))
    c, buf = make_console()
    assert cli.render_serve(c, "qwen3:0.6b") == 1
    assert "isn't in the configured Ollama store" in buf.getvalue()
...
def test_serve_refuses_new_custom_name_for_missing_model_without_pull(
        make_console, monkeypatch):
    _wire_serve(monkeypatch, names=("other:1",))
...
def test_serve_explicit_ctx_requires_reusable_measurement(make_console, monkeypatch):
    ...
    assert "reusable measured ceiling" in buf.getvalue()
...
def test_serve_unmeasured_refuses_analytic_estimate(make_console, monkeypatch):
    _wire_serve(monkeypatch, characterization=None)
    monkeypatch.setattr(cli, "_ollama_estimated_ceiling",
                        lambda *_a, **_k: pytest.fail("serve consumed analytic estimate"))
    c, buf = make_console()
    assert cli.render_serve(c, "qwen3:0.6b") == 1
    assert "reusable measured ceiling" in buf.getvalue()
...
def test_serve_ignores_measurement_for_retargeted_ollama_manifest(
    ...
    _wire_serve(monkeypatch, characterization={"safe_context": 4096,
                                               "artifact_id": old_artifact})
    monkeypatch.setattr(cli, "_ollama_estimated_ceiling",
                        lambda *_a, **_k: pytest.fail("replaced stale measurement with estimate"))
    c, buf = make_console()
    assert cli.render_serve(c, "qwen3:0.6b") == 1
    assert "reusable measured ceiling" in buf.getvalue()
...
def test_serve_refuses_if_base_manifest_retargets_during_load(make_console, monkeypatch):
    ...
    c, buf = make_console()
    assert cli.render_serve(c, "qwen3:0.6b", ctx=8192) == 1
    assert "manifest changed during OpenAI contract verification" in buf.getvalue()
...
def test_serve_reuses_exact_owned_manifest_after_daemon_eviction(
    ...
    observations = [[], [resident], [resident]]
    monkeypatch.setattr(cli.ollama, "processes", lambda: observations.pop(0))
    monkeypatch.setattr(
...
def test_serve_happy_json(make_console, monkeypatch, capsys):
    ...
    assert payload["openai_contract_verified"] is True


def test_serve_probes_openai_contract_with_exact_derived_model(
        make_console, monkeypatch):
    _wire_serve(monkeypatch, ps_rows=_SERVE_LOADED)
    probes = []
    monkeypatch.setattr(
        cli.ollama, "openai_completion_probe",
        lambda model: probes.append(model) or True,
    )
    c, _ = make_console()

    assert cli.render_serve(c, "qwen3:0.6b", ctx=8192) == 0
    assert probes == [_serve_name()]


def test_serve_refuses_and_cleans_up_when_openai_contract_fails(
        make_console, monkeypatch):
    _wire_serve(monkeypatch, ps_rows=_SERVE_LOADED)
    monkeypatch.setattr(cli.ollama, "openai_completion_probe", lambda _name: False)
    deleted = []
    monkeypatch.setattr(cli.ollama, "delete", lambda name: deleted.append(name) or True)
    c, buf = make_console()

    assert cli.render_serve(c, "qwen3:0.6b", ctx=8192) == 1
    assert "OpenAI /v1 completion contract" in buf.getvalue()
    assert deleted == [_serve_name()]


def test_serve_refuses_when_context_changes_during_openai_probe(
        make_console, monkeypatch):
    _wire_serve(monkeypatch, ps_rows=_SERVE_LOADED)
    safe = cli.ollama.OllamaProcess(
        name=_serve_name(), digest="b" * 64, size_bytes=100, size_vram_bytes=100,
        effective_context_per_request=8192,
    )
    changed = dataclasses.replace(safe, effective_context_per_request=16384)
    observations = [[], [safe], [changed]]
    monkeypatch.setattr(cli.ollama, "processes", lambda: observations.pop(0))
    c, buf = make_console()
...
def test_serve_refuses_when_runtime_changes_during_openai_probe(
        make_console, monkeypatch):
    _wire_serve(monkeypatch, ps_rows=_SERVE_LOADED)
    initial = cli.ollama.runtime_authority(
        cli.ollama.OllamaEndpoint("http://127.0.0.1:11434", "loopback"))
    observations = iter([
        initial,
        dataclasses.replace(initial, server_instance_id="replacement"),
    ])
    monkeypatch.setattr(cli.ollama, "runtime_authority", lambda _endpoint: next(observations))
    c, buf = make_console()

    assert cli.render_serve(c, "qwen3:0.6b", ctx=8192) == 1
    assert "runtime changed during OpenAI contract verification" in buf.getvalue()


def test_serve_refuses_without_cleanup_when_served_manifest_changes_during_openai_probe(
        make_console, monkeypatch):
    _wire_serve(monkeypatch, ps_rows=_SERVE_LOADED)
    served_reads = iter(["b" * 64, "b" * 64, "c" * 64])
    monkeypatch.setattr(
        cli.ollama, "manifest_digest",
        lambda name: "a" * 64 if name == "qwen3:0.6b" else next(served_reads),
    )
    monkeypatch.setattr(
        cli.ollama, "delete", lambda *_a, **_k: pytest.fail("deleted retargeted manifest"))
    c, buf = make_console()

    assert cli.render_serve(c, "qwen3:0.6b", ctx=8192) == 1
    assert "changed during OpenAI contract verification" in buf.getvalue()
    assert "did not unload or delete it" in buf.getvalue()
...
def test_named_serve_never_passes_inventory_record_to_estimator(make_console, monkeypatch):
    ...
    c, buf = make_console()
    assert cli.render_serve(c, "qwen3:0.6b") == 1
    assert seen == [] and "reusable measured ceiling" in buf.getvalue()
...
# Serving consumes characterization evidence but never rewrites it.
...
def test_serve_unknown_residency_is_refused_and_not_saved_as_measured(
        make_console, monkeypatch, capsys):
    rows = [{"name": "qwen3-0.6b-ara:latest", "context_length": 8192}]
    _wire_serve(monkeypatch, characterization={"safe_context": 8192}, ps_rows=rows)
    saved = _wire_save_recorder(monkeypatch)
    c, _ = make_console()
